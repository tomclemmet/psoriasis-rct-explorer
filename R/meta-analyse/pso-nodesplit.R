...
pso_nodesplit <- function(
  ...
  data$split <- c(match(pasi_id_nodesplit$drug, nodesplit[1]),
                 match(pasi_id_nodesplit$drug, nodesplit[2]))
...
model {
  ...
  delta_re <- "
    for (k in 2:na[i]) {                                                        # LOOP THROUGH ARMS
      ...
      split[i, k] <- equals(t[i, 1], split[1]) * equals(t[i, k], split[2]) +
                      equals(t[i, 1], split[2]) * equals(t[i, k], split[1])
      md[i, k] <- (1 - split[i, k]) * (d[t[i, k]] - d[t[i, 1]] + sw[i, k]) +   # mean of LHR distributions, with multi-arm trial correction
                   split[i, k] * ddir                                                         
  ...
  delta_fe <- "
    for (k in 2:na[i]) {                                                        # LOOP THROUGH ARMS
      split[i, k] <- equals(t[i, 1], split[1]) * equals(t[i, k], split[2]) +
                      equals(t[i, 1], split[2]) * equals(t[i, k], split[1]) 
      delta[i, k] <- (1 - split[i, k]) * (d[t[i, k]] - d[t[i, 1]]) +
                   split[i, k] * ddir
    }
    resdev[i] <- sum(dev[i, 1:na[i]])                                           # summed residual deviance contribution for this trial
  }"
...
"
  ...
  model_code <- paste0(
    ...
    switch(cutpoints,
           ...
           "trial" = rez_trial,
           "arm" = rez_arm),
    if (class == "exchangeable") d_priors_class else d_priors,
    priors,
    if (cutpoints != "treatment") probs_fez else probs_rez_tx,
    end
  )
  
  if(is.na(filename)) {
    filename <- paste0("JAGS/nodesplit_", paste(
  ...
  message(paste0("Fitting ", "nodesplit NMA for PASI response with ", effects, 
  ...
  posterior::as_draws_df(mod$BUGSoutput$sims.array) |> 
    select(-starts_with(".")) |> suppressWarnings() |> 
    pivot_longer(everything(), names_to = "label", values_to = "d") |> 
    ggplot() +
    geom_density(aes(d, fill = label), alpha = 0.5)
}
